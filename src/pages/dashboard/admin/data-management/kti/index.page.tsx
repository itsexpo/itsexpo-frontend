...
import { KTIDataRecap } from '@/pages/dashboard/admin/data-management/kti/components/KTIDataRecap';
import { PaginatedApiResponse } from '@/types/api';
import {
  AdminKTIColumn,
  KTIDataRecapType,
} from '@/types/entities/data-management/kti';

export default withAuth(AdminKTIDashboardPage, ['admin_kti.index']);

function AdminKTIDashboardPage() {
  const router = useRouter();
  const { tableState, setTableState } = useServerTable<AdminKTIColumn>({
    pageSize: 10,
  });

  const columns: ColumnDef<AdminKTIColumn>[] = [
    {
      id: 'index',
        ...
        return (
          <div className='flex flex-row justify-center'>
            <Button
              ...
              onClick={() => router.push(`kti/${info.row.original.id_tim}`)}
            >
              Lihat Detail
            </Button>
          </div>
  ...
  const { data: queryData } = useQuery<
    PaginatedApiResponse<AdminKTIColumn[]>,
    Error
  >([url], {
      ...
      <main>
        <section className='dashboard-layout'>
          <div className='min-h-screen flex flex-col gap-6 pb-20'>
            ...
            <KTIDataRecap
              {...(queryData?.data.meta as unknown as KTIDataRecapType)}
            />
            <ServerTable
              columns={columns}
