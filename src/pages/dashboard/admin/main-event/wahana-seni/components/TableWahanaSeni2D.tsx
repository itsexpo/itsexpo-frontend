...
export const TableWahanaSeni2D = ({
  ...
  const { tableState, setTableState } = useServerTable<AdminWahanaSeni2DColumn>(
    {
      pageSize: 10,
    }
  );

  const columns: ColumnDef<AdminWahanaSeni2DColumn>[] = [
    {
      id: 'index',
        ...
        return (
          <div className='flex flex-row justify-center'>
            <ButtonLink
              ...
              href={`wahana-seni/${contest}/${info.row.original.id}`}
              target='_blank'
              openNewTab={true}
            >
              Lihat Detail
  ...
  const { data: queryData, isLoading } = useQuery<
    PaginatedApiResponse<AdminWahanaSeni2DColumn[]>,
    Error
  >([url], {
