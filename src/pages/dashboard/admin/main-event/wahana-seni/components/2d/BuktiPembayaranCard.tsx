...
import { JurnalistikVerification } from '@/types/entities/pre-event/jurnalistik';
...
export default function BuktiPembayaranCard({
  ...
  >(
    useMutation((data) =>
      api.patch('/admin/2d', data).then((res) => {
        onSuccess();
        return res;
      })
    )
      ...
      {tim.payment.payment_status != 'AWAITING PAYMENT' && (
        <FormProvider {...methods}>
          <div className='flex flex-col gap-0'>
            ...
            <Input
              label='Harga'
              value={formatToRupiah(parseInt(tim.payment.payment_harga))}
              readOnly
              id='harga'
              inputClassName='text-black'
            />
