...
type JoinTeamKTIProps = {
  code_team: string;
};

export default withAuth(JoinTeamKTI, []);

function JoinTeamKTI() {
  const router = useRouter();

  const methods = useForm<JoinTeamKTIProps>();
  const {
    handleSubmit,
    formState: { isDirty },
  } = methods;
  ...
  const onSubmit = (data: JoinTeamKTIProps) => {
    joinTeam(data, {
      onSuccess: () => {
